...
- インスタンス数は既定で**最小 0 台・最大 1 台**です（Terraform の `min_instances` / `max_instances`）。
  大量にアクセスされても 1 台以上に増えないので課金に上限がかかり、使われていないときは 0 台で費用がかかりません。
  1 台で同時 80 リクエストまで処理し、超えた分は 429 になります（アドインは「確認できませんでした」を表示）。
  利用者が多く 429 が出るようなら `max_instances` を増やしてください。
- Cloud Run はコールドスタートがあるため（1〜3 秒程度）、「確認できませんでした」が頻繁に出るようなら
  `min_instances = 1`（Terraform）または `--min-instances=1`（gcloud）を設定してください（常時課金になります）。
